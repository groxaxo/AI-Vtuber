...
class Audio:
    ...
    def __init__(self, config_path, type=1):
        ...
        if self.only_play_copywriting_thread == None:
            ...
            self.only_play_copywriting_thread = threading.Thread(
                target=self.start_only_play_copywriting
            )
    ...
    def stop_audio(
        self,
        type: str = "pygame",
        mixer_normal: bool = True,
        mixer_copywriting: bool = True,
    ):
        try:
            if type == "pygame":
                if mixer_normal:
    ...
    def is_queue_less_or_greater_than(
        self, type: str = "message_queue", less: int = None, greater: int = None
    ):
        if less:
            if type == "voice_tmp_path_queue":
    ...
    def search_files(self, root_dir, target_file="", ignore_extension=False):
        ...
        target_for_comparison = (
            os.path.splitext(target_file)[0] if ignore_extension else target_file
        )
        ...
        for root, dirs, files in os.walk(root_dir):
            for file in files:
                ...
                file_to_compare = (
                    os.path.splitext(file)[0] if ignore_extension else file
                )
    ...
    def get_dir_audios_filename(self, audio_path, type=0):
        ...
        try:
            ...
            if type == 1:
                # 只返回文件名不含拓展名
                file_names = [
                    os.path.splitext(os.path.basename(file))[0] for file in audio_files
                ]
            else:
                # 返回完整文件名
        ...
        while True:  # 无限循环，直到队列为空时退出
            try:
                ...
                if self.config.get("visual_body") == "metahuman_stream":
                    ...
                    message["content"] = self.common.remove_extra_words(
                        message["content"],
                        message["config"]["max_len"],
                        message["config"]["max_char_len"],
                    )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, data=params) as response:
                    if response.status == 200:
                        ...
                        voice_tmp_path = self.common.get_new_audio_path(
                            self.config.get("play_audio", "out_path"), file_name
                        )
                        ...
                        logger.debug(
                            f"so-vits-svc转换完成，音频保存在：{voice_tmp_path}"
                        )
    ...
    async def ddsp_svc_api(self, audio_path=""):
        try:
            ...
            data.add_field(
                ...
                str(self.config.get("ddsp_svc", "fSafePrefixPadLength")),
            )
            data.add_field(
                "fPitchChange", str(self.config.get("ddsp_svc", "fPitchChange"))
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        logger.info(
                            f"EasyAIVtuber发送成功，返回：{json_response['status']}"
                        )
    ...
    async def digital_human_video_player_api(self, audio_path=""):
        try:
            ...
            url = urljoin(
                self.config.get("digital_human_video_player", "api_ip_port"), "/show"
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        json_response = await response.json()
                        logger.info(
                            f"digital_human_video_player发送成功，返回：{json_response['message']}"
                        )

                        return True
                    else:
                        logger.error(
                            f"digital_human_video_player发送失败，状态码：{response.status}"
                        )
                        return False
    ...
    async def live2d_TTS_LLM_GPT_SoVITS_Vtuber_api(self, audio_path=""):
        try:
            ...
            url = urljoin(
                self.config.get("live2d_TTS_LLM_GPT_SoVITS_Vtuber", "api_ip_port"),
                "/ws",
            )
            resp_json = self.common.get_filename_from_path(audio_path)
            if resp_json["code"] == 200:
                audio_url = urljoin(
                    f"http://{self.config.get('webui', 'ip')}:{self.config.get('webui', 'port')}",
                    f"/out/{resp_json['data']}",
                )
            else:
                logger.error(
                    f"live2d_TTS_LLM_GPT_SoVITS_Vtuber获取音频失败，返回：{resp_json['error']}"
                )
                return False

            data = {"action": "talk", "data": {"audio_path": audio_url}}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        json_response = await response.json()
                        logger.info(
                            f"live2d_TTS_LLM_GPT_SoVITS_Vtuber发送成功，返回：{json_response['message']}"
                        )

                        return True
                    else:
                        logger.error(
                            f"live2d_TTS_LLM_GPT_SoVITS_Vtuber发送失败，状态码：{response.status}"
                        )
                        return False
    ...
    def data_priority_insert(self, type: str = "等待合成消息", data_json: dict = None):
        ...
        if type == "等待合成消息":
            logger.info(
                f"{type} 优先级: {new_data_priority} 内容：【{data_json['content']}】"
            )
            ...
            if new_data_priority is None:
                ...
                for i in range(len(Audio.message_queue) - 1, -1, -1):
                    ...
                    if priority_level is not None:
                        ...
                        # 确保比较时排除未定义类型的元素
                        if (
                            item_priority is not None
                            and item_priority >= new_data_priority
                        ):
            ...
            if insert_position >= int(
                self.config.get("filter", "message_queue_max_len")
            ):
                logger.info(f"message_queue 已满，数据丢弃：【{data_json['content']}】")
                return {
                    "code": 1,
                    "msg": f"message_queue 已满，数据丢弃：【{data_json['content']}】",
                }
            ...
            logger.info(
                f"{type} 优先级: {new_data_priority} 音频={data_json['voice_path']}"
            )
            ...
            # 数据队列数据量超长判断，插入位置索引大于最大数，则说明优先级低与队列中已存在数据，丢弃数据
            if insert_position >= int(
                self.config.get("filter", "voice_tmp_path_queue_max_len")
            ):
                logger.info(
                    f"voice_tmp_path_queue 已满，音频丢弃：【{data_json['voice_path']}】"
                )
                return {
                    "code": 1,
                    "msg": f"voice_tmp_path_queue 已满，音频丢弃：【{data_json['voice_path']}】",
                }
            ...
            with Audio.voice_tmp_path_queue_lock:
                ...
                if (
                    len(Audio.voice_tmp_path_queue)
                    >= int(
                        self.config.get("filter", "voice_tmp_path_queue_min_start_play")
                    )
                    and Audio.voice_tmp_path_queue_not_empty_flag is False
                ):
        ...
        try:
            ...
            if self.config.get("filter", "username_convert_digits_to_chinese"):
                if message["username"] is not None:
                    message["username"] = self.common.convert_digits_to_chinese(
                        message["username"]
                    )
    ...
    async def tts_handle(self, message):
        ...
        try:
            if message["tts_type"] == "vits":
                ...
                data = {"content": message["content"], "edge-tts": message["data"]}
                ...
                if message["data"]["api_0322"]["text_lang"] == "自动识别":
                    ...
                    if language in language_name_dict:
                        message["data"]["api_0322"]["text_lang"] = language_name_dict[
                            language
                        ]
                    else:
                        message["data"]["api_0322"]["text_lang"] = (
                            "中文"  # 无法识别出语言代码时的默认值
                        )
                ...
                data = {"content": message["content"], "multitts": message["data"]}

                voice_tmp_path = await self.my_tts.multitts_api(data)
            elif message["tts_type"] == "melotts":
                data = {"content": message["content"], "melotts": message["data"]}
            ...
            elif message["tts_type"] == "custom_tts":
                data = {
                    "content": message["content"],
                    "api_ip_port": message["data"]["api_ip_port"],
                }
                voice_tmp_path = await self.my_tts.custom_tts_api(data)
    ...
    async def send_audio_play_info_to_callback(self, data: dict = None):
        ...
        try:
            ...
            main_api_ip = (
                "127.0.0.1"
                if self.config.get("api_ip") == "0.0.0.0"
                else self.config.get("api_ip")
            )
            resp = await self.common.send_async_request(
    ...
    async def my_play_voice(self, message):
        ...
        async def voice_change_and_put_to_queue(message, voice_tmp_path):
            ...
            elif message["type"] == "read_comment" and not self.config.get(
                "read_comment", "voice_change"
            ):
                # 是否开启了音频播放，如果没开，则不会传文件路径给播放队列
                if self.config.get("play_audio", "enable"):
                    self.data_priority_insert("待播放音频列表", data_json)
        ...
        if voice_tmp_path is None:
            logger.error(
                f"{message['tts_type']}合成失败，请排查服务端是否启动、是否正常，配置、网络等问题。如果排查后都没有问题，可能是接口改动导致的兼容性问题，可以前往官方仓库提交issue，传送门：https://github.com/Ikaros-521/AI-Vtuber/issues\n如果是GSV 400错误，请确认参考音频和参考文本是否正确，或替换参考音频进行尝试"
            )
        ...
        logger.info(
            f"[{message['tts_type']}]合成成功，合成内容：【{message['content']}】，音频存储在 {voice_tmp_path}"
        )
    ...
    def audio_speed_change(self, audio_path, speed_factor=1.0, pitch_factor=1.0):
        ...
        """
        logger.debug(
            f"audio_path={audio_path}, speed_factor={speed_factor}, pitch_factor={pitch_factor}"
        )
        ...
        if speed_factor > 1.0:
            ...
            audio_changed = audio._spawn(
                audio.raw_data, overrides={"frame_rate": slow_frame_rate}
            )
        else:
            audio_changed = audio
        ...
        if pitch_factor != 1.0:
            ...
            audio_changed = audio_changed._spawn(
                ...
                overrides={
                    "frame_rate": int(
                        audio_changed.frame_rate * (2.0 ** (semitones / 12.0))
                    )
                },
    ...
    async def only_play_audio(self):
        try:
            ...
            try:
                ...
                logger.error(traceback.format_exc())
                logger.error(
                    "pygame mixer_normal初始化失败，普通音频将无法正常播放，请检查声卡是否正常！"
                )
            ...
            while True:
                try:
                    ...
                    if Audio.copywriting_play_flag == 2:
                        ...
                        # 等待一个切换时间
                        await asyncio.sleep(
                            float(self.config.get("copywriting", "switching_interval"))
                        )
                    ...
                    if captions_config["enable"]:
                        ...
                        self.common.write_content_to_file(
                            captions_config["file_path"],
                            data_json["content"],
                            write_log=False,
                        )
                    ...
                    if self.config.get("web_captions_printer", "enable"):
                        await self.common.send_to_web_captions_printer(
                            self.config.get("web_captions_printer", "api_ip_port"),
                            data_json,
                        )
                    ...
                    if self.config.get(
                    ...
                    ) and "音频播放时" in self.config.get(
                        "luoxi_project", "Live_Comment_Assistant", "trigger_position"
                    ):
                        from utils.luoxi_project.live_comment_assistant import (
                            send_msg_to_live_comment_assistant,
                        )
                        ...
                        if data_json["type"] in type_mapping:
                            ...
                            # 当前消息类型是使能的触发类型
                            if tmp_type in self.config.get(
                                "luoxi_project", "Live_Comment_Assistant", "type"
                            ):
                                await send_msg_to_live_comment_assistant(
                                    self.config.get(
                                        "luoxi_project", "Live_Comment_Assistant"
                                    ),
                                    data_json["content"],
                                )

                    normal_interval_min = self.config.get(
                        "play_audio", "normal_interval_min"
                    )
                    normal_interval_max = self.config.get(
                        "play_audio", "normal_interval_max"
                    )
                    normal_interval = self.common.get_random_value(
                        normal_interval_min, normal_interval_max
                    )

                    interval_num_min = float(
                        self.config.get("play_audio", "interval_num_min")
                    )
                    interval_num_max = float(
                        self.config.get("play_audio", "interval_num_max")
                    )
                    interval_num = int(
                        self.common.get_random_value(interval_num_min, interval_num_max)
                    )
                    ...
                    if self.config.get("audio_random_speed", "normal", "enable"):
                        random_speed = self.common.get_random_value(
                            self.config.get(
                        ...
                        voice_tmp_path = self.audio_speed_change(
                            voice_tmp_path, random_speed
                        )
                    ...
                    if self.config.get("visual_body") == "xuniren":
                        ...
                        if self.config.get("play_audio", "player") in [
                            ...
                            try:
                                ...
                                logger.error(
                                    f"无法加载音频文件:{voice_tmp_path}。请确保文件格式正确且文件未损坏。可能原因是TTS配置有误或者TTS服务端有问题，可以去服务端排查一下问题"
                                )
    ...
    def delayed_execution_unpause_copywriting_play(self):
        ...
        if (
            Audio.unpause_copywriting_play_timer is not None
            and Audio.unpause_copywriting_play_timer.is_alive()
        ):
            Audio.unpause_copywriting_play_timer.cancel()

        # 创建新的计时器并启动
        Audio.unpause_copywriting_play_timer = threading.Timer(
            float(self.config.get("copywriting", "switching_interval")),
    ...
    async def only_play_copywriting(self):
        try:
            try:
                ...
                logger.error(traceback.format_exc())
                logger.error(
                    "pygame mixer_copywriting初始化失败，文案音频将无法正常播放，请检查声卡是否正常！"
                )
            ...
            async def random_speed_and_play(audio_path):
                ...
                if self.config.get("audio_random_speed", "copywriting", "enable"):
                    random_speed = self.common.get_random_value(
                        self.config.get(
                            "audio_random_speed", "copywriting", "speed_min"
                        ),
                        self.config.get(
                            "audio_random_speed", "copywriting", "speed_max"
                        ),
                    )
                ...
                if self.config.get("visual_body") == "xuniren":
                    ...
                    if self.config.get("play_audio", "player") in [
                        ...
                        data_json = {
                            ...
                            "content": audio_path,
                            "random_speed": {"enable": False, "max": 1.3, "min": 0.8},
                            "speed": 1,
                        }
                        Audio.audio_player.play(data_json)
                    else:
                ...
                # 添加延时，暂停执行n秒钟
                await asyncio.sleep(
                    float(self.config.get("copywriting", "audio_interval"))
                )
            ...
            try:
                ...
                def all_data_reload(
                    ...
                    for copywriting_config in copywriting_configs:
                        ...
                        continuous_play_num_arr.append(
                            copywriting_config["continuous_play_num"]
                        )
                ...
                while True:
                    ...
                    for index, play_list in enumerate(play_list_arr):
                        ...
                        for i in range(0, continuous_play_num_arr[index]):
                            ...
                            if (
                                float(self.common.get_bj_time(3)) - start_time
                            ) > max_play_time_arr[index]:
                                break
                            ...
                            if len(play_list) > 0:
                                ...
                                audio_path = os.path.join(
                                    audio_path_arr[index], voice_tmp_path
                                )
                                audio_path = os.path.abspath(audio_path)
    ...
    def merge_audio_files(
        self, directory, base_filename, last_index, pause_duration=1, format="wav"
    ):
        merged_audio = None
    ...
    async def audio_synthesis_use_local_config(
        self, content, audio_synthesis_type="edge-tts"
    ):
        """使用本地配置进行音频合成，返回音频路径
        ...
        if audio_synthesis_type == "vits":
            ...
            data = {"content": content, "edge-tts": self.config.get("edge-tts")}
            ...
            data = {
                "request_parameters": self.config.get(
                    "gradio_tts", "request_parameters"
                ),
                "content": content,
            }
            # 调用接口合成语音
            ...
            voice_tmp_path = await self.my_tts.index_tts_api(data)
        elif audio_synthesis_type == "custom_tts":
            data = {
                "content": content,
                "api_ip_port": self.config.get("custom_tts", "api_ip_port"),
            }
            voice_tmp_path = await self.my_tts.custom_tts_api(data)
    ...
    async def copywriting_synthesis_audio(
        self, file_path, out_audio_path="out/", audio_synthesis_type="edge-tts"
    ):
        """文案音频合成
        ...
        try:
            ...
            async def voice_change_and_put_to_queue(voice_tmp_path):
                ...
                if voice_tmp_path:
                    ...
                    self.common.move_file(
                        voice_tmp_path, out_file_path, file_name + "-" + str(file_index)
                    )
            ...
            for content in sentences:
                ...
                while retry_count > 0:
                    ...
                    try:
                        voice_tmp_path = await self.audio_synthesis_use_local_config(
                        ...
                        logger.info(
                            f"{audio_synthesis_type}合成成功，合成内容：【{content}】，输出到={voice_tmp_path}"
                        )
                        ...
                        if retry_count <= 0:
                            logger.error(
                                f"重试次数用尽，{audio_synthesis_type}合成最终失败，请排查配置、网络等问题"
                            )
                            self.abnormal_alarm_handle("tts")
                            return
    ...
    def abnormal_alarm_handle(self, type):
        ...
        try:
            ...
            logger.debug(
                f"abnormal_alarm_handle type={type}, error_count={Audio.abnormal_alarm_data[type]['error_count']}"
            )

            if self.config.get("abnormal_alarm", type, "type") == "local_audio":
                # 是否错误数大于 自动重启错误数
                if Audio.abnormal_alarm_data[type]["error_count"] >= self.config.get(
                    "abnormal_alarm", type, "auto_restart_error_num"
                ):
                    logger.warning(
                        f"【异常报警-{type}】 出错数超过自动重启错误数，即将自动重启"
                    )
                    data = {
                        "type": "restart",
                        "api_type": "api",
                ...
                if Audio.abnormal_alarm_data[type]["error_count"] < self.config.get(
                    "abnormal_alarm", type, "start_alarm_error_num"
                ):
                    return

                path_list = self.common.get_all_file_paths(
                    self.config.get("abnormal_alarm", type, "local_audio_path")
                )
                ...
                data_json = {
                    ...
                    "content": os.path.join(
                        self.config.get("abnormal_alarm", type, "local_audio_path"),
                        self.common.extract_filename(audio_path, True),
                    ),
                }

                logger.warning(
                    f"【异常报警-{type}】 {self.common.extract_filename(audio_path, False)}"
                )
